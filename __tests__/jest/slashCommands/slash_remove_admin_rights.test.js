...
afterEach(() => {
  jest.clearAllMocks();
  userModelInstanceMock.admin = true;
});

describe("slash remove_admin_rights", () => {
  ...
  test("Saves admin value if user exists", async () => {
    ...
    expect(editEphemeral).toHaveBeenCalledWith(defaultAdminInteraction, "Removed admin rights from JonDoe.");
  });
});
