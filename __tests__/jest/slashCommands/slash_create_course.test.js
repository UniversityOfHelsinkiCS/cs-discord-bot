...
findCourseFromDbWithFullName.mockImplementation(() => false).mockImplementationOnce(() => true);
findCourseFromDb
  .mockImplementation(() => false)
  .mockImplementationOnce(() => true)
  .mockImplementationOnce(() => true);

createCourseToDatabase.mockImplementation(() => {
  return { name: "nickname", id: Math.floor(Math.random() * 10) + 5 };
});
...
describe("slash create command", () => {
  ...
  test("respond with correct emphemeral", async () => {
    ...
    expect(editEphemeral).toHaveBeenNthCalledWith(
      1,
      defaultTeacherInteraction,
      `Created course ${courseName}. Adding you as an instructor to the course.`
    );
  });
  ...
  test("fails if parameters are too long", async () => {
    defaultTeacherInteraction.options = {
      getString: jest.fn((name) => {
        const names = {
          ...
          full_name:
            "You think you can get away with saying that shit to me over the Internet? Think again, fucker. As we speak I am contacting my secret" +
          ...
          nick_name:
            "Not only am I extensively trained in unarmed combat, but I have access to the entire arsenal of the United States Marine Corps and I will use it to" +
    ...
    await execute(defaultTeacherInteraction, client, models);
    expect(sendErrorEphemeral).toHaveBeenCalledTimes(1);
    expect(sendErrorEphemeral).toHaveBeenCalledWith(
      defaultTeacherInteraction,
      "Course code, name and nickname are too long!"
    );
  });
