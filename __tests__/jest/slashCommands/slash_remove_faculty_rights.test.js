...
afterEach(() => {
  jest.clearAllMocks();
  userModelInstanceMock.faculty = true;
});

describe("slash remove_faculty_rights", () => {
  ...
  test("Saves faculty value if user exists", async () => {
    ...
    expect(editEphemeral).toHaveBeenCalledWith(defaultAdminInteraction, "Removed faculty rights from JonDoe.");
  });
});
